...
if [[ "$1" == "kafka" ]]; then
  ...
  export KAFKA_LOG_DIR=${KAFKA_LOG_DIR:-/data}
  ...
  export CONFIG_PATH=${CONFIG_PATH:-/kafka/config/kafka.properties}
  ...
  for var in ${!KAFKA_@}; do
	...
	kafka-storage format --ignore-formatted --config ${CONFIG_PATH} --cluster-id "${CLUSTER_ID}"
